...
#' A `OGREDataset` is a `GenomicRangesList` with stores datasets in a list like structure and possible metadata information.
...
OGREDataSetFromDir <- function(queryFolder,subjectFolder){
  ...
  dir.create(file.path(dirname(metadata(OGREDataSet)$queryFolder), "output")) #move up one level
  metadata(OGREDataSet)$outputFolder <- file.path(dirname(metadata(OGREDataSet)$queryFolder), "output") #move up one level
  dir.create(file.path(dirname(metadata(OGREDataSet)$outputFolder), "gvizPlots")) #move up one level
  metadata(OGREDataSet)$gvizPlotsFolder <- file.path(dirname(metadata(OGREDataSet)$outputFolder), "gvizPlots") #move up one level
